...
document.addEventListener('DOMContentLoaded', function () {
    ...
    tabLinks.forEach(function (tab) {
        tab.addEventListener('click', function () {
            ...
            if (targetPage === 'achievement') {
                loadAchievements();
            }
        });

        // Tombol reset quiz
        document.getElementById('reset-quiz-button').addEventListener('click', function () {
            app.dialog.confirm(
                'Yakin ingin mereset semua progress quiz?',
                'Konfirmasi Reset',
                function () {
                    resetQuizProgress();
                }
...
function loadProfileStats() {
    const scores = [
        parseInt(localStorage.getItem('quiz1Score')) || 0,
        parseInt(localStorage.getItem('quiz2Score')) || 0,
        parseInt(localStorage.getItem('quiz3Score')) || 0
    ];

    const totalQuiz = scores.length;

    // Hitung quiz yang sudah dikerjakan
    const quizCompleted = scores.filter(score => score > 0).length;

    // Progress (%)
    const progress = Math.round((quizCompleted / totalQuiz) * 100);

    // Hitung achievement
    let achievements = 0;
    scores.forEach(score => {
        if (score >= 50) achievements++;
    });

    // Update tampilan profil
    document.getElementById('profile-progress').innerText = progress + '%';
    document.getElementById('profile-materi').innerText = quizCompleted;
    document.getElementById('profile-quiz').innerText = quizCompleted;
    document.getElementById('profile-achievement').innerText = achievements;
}
...
function resetQuizProgress() {
    ...
    alert('Progress quiz berhasil direset!');
}
